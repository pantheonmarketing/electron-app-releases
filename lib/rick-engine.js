...
function copyScript(script) {
  ...
  const importedText = cleanText(script.importedText, 24000);
...
function addScriptVersion(session, source = 'revision') {
  ...
  if (last && JSON.stringify(last.script) === JSON.stringify(script)) {
    if (versions.length === 1 && last.id === ORIGINAL_VERSION_ID && source === 'imported') last.source = 'imported';
...
function validateImportedScript(value) {
  if (typeof value !== 'string') throw new Error('Paste a script or choose a text file');
  if (value.length > 24000) throw new Error('Keep imported scripts under 24,000 characters');
  const importedText = value
    .replace(/\r\n/g, '\n')
    .replace(/[ \t]+/g, ' ')
    .replace(/ *\n */g, '\n')
    .replace(/\n{3,}/g, '\n\n')
    .trim();
  if (!importedText) throw new Error('Paste a script or choose a text file');
  const wordCount = importedText.split(/\s+/).filter(Boolean).length;
  if (wordCount > 3200) throw new Error('Keep imported scripts under 3,200 words');
  return { importedText };
}

function recordingScriptText(script = {}) {
  const imported = cleanText(script.importedText, 24000);
  if (imported) return imported;
  return RECORDING_SECTIONS
    .map(([section]) => cleanText(script[section], 6000))
    .filter(Boolean)
    .join('\n\n');
}
...
function createRecordingScenes(script = {}, maxWords = 42) {
  ...
  const scenes = [];
  const recordingSections = cleanText(script.importedText, 24000)
    ? [['importedText', 'Script']]
    : RECORDING_SECTIONS;
  for (const [section, sectionLabel] of recordingSections) {
    const source = cleanText(script[section], section === 'importedText' ? 24000 : 6000);
    const words = [...source.matchAll(/\S+/g)];
    if (!words.length) continue;
    const sectionLimit = section === 'importedText' ? Math.min(80, Math.max(limit, Math.ceil(words.length / 40))) : limit;
    const chunks = [];
    for (let index = 0; index < words.length; index += sectionLimit) {
      const first = words[index];
      const last = words[Math.min(index + sectionLimit, words.length) - 1];
      chunks.push({
        // Slice from the original wording instead of joining tokens so commas,
        // intentional line breaks, and paragraph spacing reach the prompt as written.
        text: source.slice(first.index, last.index + last[0].length),
        wordCount: Math.min(sectionLimit, words.length - index),
      });
    }
...
function validateRecordingScenes(value) {
  const source = Array.isArray(value) ? value : value?.scenes;
  if (!Array.isArray(source) || !source.length || source.length > 40) {
    throw new Error('Choose between 1 and 40 recording scenes');
  }
  return source.map((scene, index) => {
    const text = cleanSpokenText(scene?.text);
    if (!text) throw new Error(`Scene ${index + 1} needs some text`);
    const label = cleanText(scene?.label, 80) || `Beat ${index + 1}`;
    const section = cleanText(scene?.section, 40) || 'custom';
    return {
      id: cleanText(scene?.id, 120) || `custom-${index + 1}`,
      section,
      label,
      text,
...
function validateAiRecordingScenes(value, script) {
  const scenes = validateRecordingScenes(value);
  const expected = recordingScriptText(script)
    .replace(/\s+/g, ' ')
    .trim();
  const actual = scenes
    ...
    .trim();
  if (actual !== expected) {
    throw new Error('Rick changed the script while splitting it into scenes');
  }
...
function normalizeTargetDuration(value) {
  const normalized = cleanText(value, 20).toLowerCase();
  return Object.prototype.hasOwnProperty.call(DURATION_PRESETS, normalized) ? normalized : 'standard';
}

function setTargetDuration(session, value) {
  session.targetDuration = normalizeTargetDuration(value);
  return setUpdated(session);
}
