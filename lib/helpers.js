...
// Simple write lock to prevent concurrent writeTasks() calls from corrupting data
...
function writeTasks(tasks) {
  if (_writeLock) {
    // Queue write and process after current one finishes
    return new Promise(resolve => {
      _writeQueue.push(() => { _writeTasksSync(tasks); resolve(); });
    });
  }
  _writeLock = true;
  try {
...
function getNodeOutput(node) {
  ...
  const resultFile = path.join(shared.RESULTS_DIR, `task-${node.task_id}.json`);
...
function launchWorkersInternal(workerCount) {
  ...
  for (let i = 1; i <= actual; i++) {
    ...
    if (shared.launchWorkerProcess(workerScript, String(i), null, cleanEnv)) {
